...
public class LevelSix extends AppCompatActivity {
    ...
    private ImageView bag, home, pause, square, circle, triangle, hexagon, pentagon, rectangle, star;
    ...
    private float hexagonX, hexagonY;
    private float pentagonX, pentagonY;
    ...
    public void changePos() {
        ...
        // Hexagon
        hexagonY += 13;
        float hexagonCenterX = hexagonX + hexagon.getWidth() / 2;
        float hexagonCenterY = hexagonY + hexagon.getHeight() / 2;
        if (hitCheck(hexagonCenterX, hexagonCenterY)) {
            hexagonY = frameHeight + 100;
            lives -= 1;
            sound.playHitShapeSound();
            if (lives == 0) {
                gameOver();
        ...
        if (hexagonY > frameHeight) {
            hexagonY = -100;
            hexagonX = (float)Math.floor(Math.random() * (frameWidth - hexagon.getWidth()));
        }
        hexagon.setX(hexagonX);
        hexagon.setY(hexagonY);
        // Pentagon
        pentagonY += 14;
        float pentagonCenterX = pentagonX + pentagon.getWidth() / 2;
        float pentagonCenterY = pentagonY + pentagon.getHeight() / 2;
        if (hitCheck(pentagonCenterX, pentagonCenterY)) {
            pentagonY = frameHeight + 100;
            lives -= 1;
            sound.playHitShapeSound();
            if (lives == 0) {
                gameOver();
        ...
        if (pentagonY > frameHeight) {
            pentagonY = -100;
            pentagonX = (float)Math.floor(Math.random() * (frameWidth - pentagon.getWidth()));
        }
        pentagon.setX(pentagonX);
        pentagon.setY(pentagonY);
        // Rectangle
        rectangleY += 15;
        float rectangleCenterX = rectangleX + rectangle.getWidth() / 2;
        float rectangleCenterY = rectangleY + rectangle.getHeight() / 2;
        if (hitCheck(rectangleCenterX, rectangleCenterY)) {
            rectangleY = frameHeight + 100;
            lives -= 1;
            sound.playHitShapeSound();
            if (lives == 0) {
                gameOver();
        ...
        if (rectangleY > frameHeight) {
            rectangleY = -100;
            rectangleX = (float)Math.floor(Math.random() * (frameWidth - rectangle.getWidth()));
        }
    ...
    public void startGame(View view) {
        ...
        hexagon.setY(3000.0f);
        pentagon.setY(3000.0f);
        rectangle.setY(3000.0f);
        star.setY(3000.0f);
        squareY = square.getY();
        circleY = circle.getY();
        triangleY = triangle.getY();
        hexagonY = hexagon.getY();
        pentagonY = pentagon.getY();
        rectangleY = rectangle.getY();
        starY = star.getY();
        ...
        pentagon.setVisibility(View.VISIBLE);
